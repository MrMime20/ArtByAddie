/* ==========================================================================
   ...
   External editor tool for Addie Sagers Portfolio
   ========================================================================== */
...
const DEFAULT_STATE = {
    ...
    aboutData: {
        ...
        paragraphs: [
            "Addie Sagers is an artist working across digital illustration, watercolor, and mixed media. Her work explores tension between the organic and the synthetic — soft washes of colour pressed against hard-edged geometry, handmade textures layered beneath glitch and noise.",
            "Based in the US, she draws influence from science fiction concept art, traditional printmaking, and the quiet strangeness of everyday objects. Each piece begins as an intuition and ends somewhere unexpected."
        ],
        tags: [
...
const STORAGE_KEY = 'addie_portfolio_editor_state_v2';
...
function saveState() {
    try {
        ...
        if (statusEl) {
            statusEl.innerHTML = '<i data-lucide="check-circle-2"></i> Auto-saved ' + new Date().toLocaleTimeString([], {
                hour: '2-digit',
                minute: '2-digit'
            });
            refreshLucideIcons();
...
function renderCoverPreview() {
    ...
    if (titleEl) titleEl.textContent = state.coverData.title || 'Addie Sagers';
...
function renderAboutPreview() {
    ...
    if (previewHeading) previewHeading.textContent = state.aboutData.heading || 'About Addie';
...
function setupGlobalActions() {
    ...
    if (exportBtn) {
        exportBtn.addEventListener('click', () => {
            ...
            dlAnchorElem.setAttribute("download", `addie_portfolio_backup_${new Date().toISOString().slice(0,10)}.json`);
            dlAnchorElem.click();
